...
# taken from https://github.com/mosco/graphknn
# citation: Amit Moscovich, Ariel Jaffe, Boaz Nadler Minimax-optimal semi-supervised regression on unknown manifolds, AISTATS (2017)
...
def algorithm1earlyexit(W, mask, k):
    '''
    Input:
    ...
    Output:
        ...
        For vertices not belonging to the terminal set the list kNN[i] is allowed to be incomplete.

    '''
    l = 0
    n_earlyexit = np.count_nonzero(mask) * k 
    ...
    W_indices = W.indices
    W_data = W.data
    while len(heap) > 0 and l < n_earlyexit:
